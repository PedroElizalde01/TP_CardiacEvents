...
<a href="https://jamanetwork.com/journals/jama/article-abstract/184989">(Torpy JM, Burke AE, Glass RM. Coronary Heart Disease Risk Factors. JAMA. 2009)</a>
...
### Gender distribution
Patients admitted with coronary disease represent more than 80% of the dataset. Males are considered to have an increased risk in developing coronary disease 
<a href="https://jamanetwork.com/journals/jama/article-abstract/184989 ">(Torpy JM, Burke AE, Glass RM. Coronary Heart Disease Risk Factors. JAMA. 2009)</a> 
...
plot_ly(alpha = 0.6,
        marker = list(color = c('rgba(0,80,99,1)', 'rgba(0,172,177,1)'), name = c("Angioplasty","Surgery"))) %>%
...
This graph shows that patien ts with one or two compromised vessels were more likely to have an angioplasty performed rather than a surgery in more than 80% of the cases. In patients with three compromised vessels were more likely to have a surgery rather than an angioplasty done in more than 60% of the cases. According to clinical trials, generally the increase of compromised vessels leads to perform a surgery on the patient, whereas patients with few compromised vessels taken into an angioplasty procedure. <a href="https://www.nejm.org/doi/full/10.1056/nejmoa1110717">(William S. Weintraub, M.D.. Comparative Effectiveness of Revascularization Strategies. The New England Journal of Medicine. 2012)</a>
...
fig1 <- plot_ly(x = ~vasos1mCoronarioDataset$x, y = ~vasos1mCoronarioDataset$y*2608.820422, type = 'scatter', mode = 'lines', name = '1 vessel', fill = 'tozeroy')%>% 
  add_trace(x = ~vasos2mCoronarioDataset$x, y = ~vasos2mCoronarioDataset$y*2608.820422, name = '2 vessels', fill = 'tozeroy')%>%
  add_trace(x = ~vasos3mCoronarioDataset$x, y = ~vasos3mCoronarioDataset$y*2608.820422, name = '3 vessels', fill = 'tozeroy')%>%
  layout(xaxis = list(title = 'Age'),yaxis = list(title = 'Density'))
...
fig2<-plot_ly(type = 'scatter', mode = 'lines', showlegend=FALSE)%>% 
  add_trace(x = ~vasos1fCoronarioDataset$x, y = ~vasos1fCoronarioDataset$y*2019.196093, name = '1 vaso', fill = 'tozeroy', line = list(color = 'rgba(31,119,180,1)'),fillcolor = 'rgba(31,119,180,0.5)')%>%
  add_trace(x = ~vasos2fCoronarioDataset$x, y = ~vasos2fCoronarioDataset$y*2019.196093, name = '2 vasos', fill = 'tozeroy', line = list(color = 'rgba(255,127,14,1)'),fillcolor = 'rgba(255,127,14,0.5)')%>%
  add_trace(x = ~vasos3fCoronarioDataset$x, y = ~vasos3fCoronarioDataset$y*2019.196093, name = '3 vasos', fill = 'tozeroy', line = list(color = 'rgba(44,160,44,1)'),fillcolor = 'rgba(44,160,44,0.5)')%>%
  layout(xaxis = list(title = 'Age'),yaxis = list(title = 'Density'))
